...
DistanceMode VL53L1X_getDistanceMode(VL53L1X* vl)
{
	return vl->distance_mode;
}

uint16_t VL53L1X_readRangeContinuousMillimeters(VL53L1X* vl, int blocking)
{
	return read(vl, blocking);
}
...
uint16_t VL53L1X_getTimeout(VL53L1X* vl)
{
	return vl->io_timeout;
}

// Record the current time to check an upcoming timeout against
void VL53L1X_startTimeout(VL53L1X* vl)
{
	vl->timeout_start_ms = millis();
}

// Check if timeout is enabled (set to nonzero value) and has expired
int VL53L1X_checkTimeoutExpired(VL53L1X* vl)
{
...
float VL53L1X_countRateFixedToFloat(VL53L1X* vl, uint16_t count_rate_fixed)
{
